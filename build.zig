...
) void 
{
    ...
    // This creates another `std.Build.Step.Compile`, but this one builds an
    // executable rather than a static library.
    ...
    // This *creates* a Run step in the build graph, to be executed when
    // another step is evaluated that depends on it. The next line below will
    ...
    // menu, and can be selected like this: `zig build run` This will evaluate
    // the `run` step rather than the default, which is "install".
